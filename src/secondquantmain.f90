Program tddftinversion
  ...
  doloop:do loop=1,nadvancesteps
     ct=sysparams%ct
     if (loop.le.4) then
        sysparams%dt=sysparams%dth/16.d0!/64
     end if
     dt=sysparams%dt
     print*,' '
     ...
     call diagdonerdm(sysparams,sharedvals,ntot1,fullvals%psi,fullvals%v,dnx)
     ! calculate full wf density
     !call fullwf_density(sysparams,fullvals%psi,dpe)
     call diagonerdm(sysparams,ntot1,fullvals%psi,dpe)
     !calculate 2nd derivative of density
     !call calcddnx(sysparams,ntot1,fullvals%psi,fullvals%v,ddnx)
     call diagddonerdm(sysparams,sharedvals,ntot1,fullvals%psi,fullvals%v,ddnx)
