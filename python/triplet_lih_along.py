...
while error > 1.e-12 and iter < 50:
    ...
    print(e)
    error = 2-(np.abs(psi1.dot(psinew[:,0]))+np.abs(psi2.dot(psinew[:,1])))
    psi1=psinew[:,0]
    psi2=psinew[:,1]
...
KSvals=td.initial_states.initializekssystem(sysparams,sharedvals,dpe,cutoff=0.1)
KSvals.vks = sharedvals.v1[:]
KSvals2=td.initial_states.initializekssystem(sysparams,sharedvals,dpe,cutoff=0.1)
...
def func(vp, t, rdm):
    ...
    return first_part - e[0]-e[1]
def jac(vp, t, rdm):
    jacvals = np.zeros(len(vp))
    e, phi = eigsh(t + np.diag(vp/np.sqrt(np.diag(rdm))), k=2, which='SA')
    for i, v in enumerate(vp):
        jacvals[i] = rdm[i,i]/np.sqrt(rdm[i,i])
        jacvals[i] -= (phi[i,0]**2+phi[i,1]**2)/np.sqrt(rdm[i,i])
    return jacvals
...
print(np.linalg.norm((phi[:,0]*phi[:,0]+phi[:,1]*phi[:,1]).flatten().real-goal_d))
print(1*np.dot(phi[:,0],sysparams.t@phi[:,0]+sharedvals.v1*phi[:,0])-goale)
exc = 1*np.dot(phi[:,0],sysparams.t@phi[:,0]+sharedvals.v1*phi[:,0])+1*np.dot(phi[:,1],sysparams.t@phi[:,1]+sharedvals.v1*phi[:,1])-goale
...
#pl.ylim((-1.,3.5))
fsum = 0.
fullvals.psi = waves[:, 1]
sysparams.energy = np.dot(fullvals.psi.conj(), t2.dot(fullvals.psi) + (vin+vks)*fullvals.psi).real #-fsum/sysparams.dx*10
sysparams.energy = (np.vdot(KSvals.phi[:,0],sysparams.t@KSvals.phi[:,0]+vext_start1*KSvals.phi[:,0])+
                    np.vdot(KSvals.phi[:,1],sysparams.t@KSvals.phi[:,1]+vext_start1*KSvals.phi[:,1])+fsum)
exacte[0] = eigsh(t2+np.diag(vin+vks), k=2, which='SA')[0][1]
...
with writer.saving(fig, "potential.mp4", 100):
    for loop in range(n_steps+1):
        ...
        exacte[pos] = eigsh(t2+np.diag(vin+vksnew), k=2, which='SA')[0][1]

        # Reset energy
        v1cur = adiapot1(sysparams.ct)[0]
        fsum += sysparams.dt*np.dot(dnx, KSvals.vks-v1cur)
        sysparams.energynew = (np.vdot(KSvals.phi[:,0],sysparams.t@KSvals.phi[:,0]+(v1cur)*KSvals.phi[:,0]).real+
                               np.vdot(KSvals.phi[:,1],sysparams.t@KSvals.phi[:,1]+(v1cur)*KSvals.phi[:,1]).real+fsum)
        ...
        energies[pos] =  (np.vdot(KSvals.phi[:,0],sysparams.t@KSvals.phi[:,0]+(v1cur)*KSvals.phi[:,0]).real+
                          np.vdot(KSvals.phi[:,1],sysparams.t@KSvals.phi[:,1]+(v1cur)*KSvals.phi[:,1]).real+fsum-exc)
        potentials[pos] = KSvals.vks.copy()
...
print('goal diff', goalf[1]-goali[1])

td.density.fullwf_density(sysparams,wavesf[:,1],dpenew)
td.propagate.shift_phases(np1, 1, np1, 2, sysparams.t, dnx*0, KSvals2.phi)
td.propagate.calculate_vks(20, np1, 1, np1, 2, sysparams.t, sysparams2.pinv0minresqlp1, ddnx*0, KSvals2.phi, KSvals2.vks)
E, V = np.linalg.eigh(sysparams.t+np.diag(KSvals.vks))
print(np.linalg.norm(dpenew-V[:,0]**2-V[:,1]**2))
E, V = np.linalg.eigh(sysparams.t+np.diag(KSvals2.vks))
print(np.linalg.norm(dpenew-V[:,0]**2-V[:,1]**2))
print(E[0]*2)
print('over', np.abs(np.dot(wavesf[:,1], psinew)))
goales,waves = eigsh(t2+np.diag(vin+vksnew), k=2, which='SA')
print(goales)
print(goalf)
print(np.dot(V[:,0],sysparams.t@V[:,0]+vext_final1*V[:,0])+np.dot(V[:,1],sysparams.t@V[:,1]+vext_final1*V[:,1])-goales[1]-exc-fsum)
print(np.dot(V[:,0],sysparams.t@V[:,0]+vext_final1*V[:,0])+np.dot(V[:,1],sysparams.t@V[:,1]+vext_final1*V[:,1])-goales[1]-exc+fsum)

import pandas as pd

dff_t = pd.DataFrame(funcvals)
dfd_t = pd.DataFrame(densities)
dfdd_t = pd.DataFrame(ddensities)
dfddd_t = pd.DataFrame(dddensities)
de_t = pd.DataFrame(energies)
dee_t = pd.DataFrame(exacte)
dvks_t = pd.DataFrame(potentials)

dff_t.to_csv('func_t.csv')
dfd_t.to_csv('dens_t.csv')
dfdd_t.to_csv('ddens_t.csv')
dfddd_t.to_csv('dddens_t.csv')
de_t.to_csv('ener_t.csv')
dvks_t.to_csv('vks_t.csv')
dee_t.to_csv('exact_ener_t.csv')